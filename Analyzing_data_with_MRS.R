...
setwd("H:/DEdelman/Training/edX Training/Analyzing Big Data with R (DAT213x)")
...
### runif = uniform distro; along with u < .05 used to limit to 5% of subset
### .rxNumRows is the size of the chunk being processed by RevoScaleR;
### runif(.rxNumRows) makes a uniform dist over the chunk
...
p + geom_histogram(aes(x = fare_amount, fill = trip_duration <= 90*60), binwidth = 10) +
...
res <- bind_cols(data.frame(rxc1, rxc2, rxc3))
...
# Generally faster than rxDataStep since it is only affecting metadata
# However, only runs locally
...
res <- bind_cols(data.frame(res1, res2))
